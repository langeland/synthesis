...
$pathKickstarter = t3lib_extMgm::extPath ( 'synthesis' );
require_once ($pathKickstarter . 'class.tx_synthesis_compilefiles.php');
require_once ($pathKickstarter . 'class.tx_synthesis_reservedwords.php');
 ...
 */
class tx_synthesis_wizard extends tx_synthesis_compilefiles {
	var $varPrefix = 'synthesis'; // redundant from 'extrep'
	var $siteBackPath = '';
	...
	function tx_synthesis_wizard() {
		...
		// init reserved words
		$resWords = t3lib_div::makeInstance ( 'tx_synthesis_reservedWords' );
	 ...
	 *
	 * @return	HTML code for the synthesis containing the module content
	 */
	function mgm_wizard() {
		$this->wizard = & $this;
		$this->initWizArray ();
		$this->sections = $GLOBALS ['TYPO3_CONF_VARS'] ['EXTCONF'] ['synthesis'] ['sections'];
		

		/* HOOK: Place a hook here, so additional things can be done */
		if (is_array ( $GLOBALS ['TYPO3_CONF_VARS'] ['EXTCONF'] ['synthesis'] ['wizard_beforeSectionsHook'] )) {
			foreach ( $GLOBALS ['TYPO3_CONF_VARS'] ['EXTCONF'] ['synthesis'] ['wizard_beforeSectionsHook'] as $_funcRef ) {
				$conf = array (
						'pObj' => $this 
				);
	...
	function sidemenu() {
		...
		if (is_array ( $GLOBALS ['TYPO3_CONF_VARS'] ['EXTCONF'] ['synthesis'] ['sidemenu'] )) {
			foreach ( $GLOBALS ['TYPO3_CONF_VARS'] ['EXTCONF'] ['synthesis'] ['sidemenu'] as $_funcRef ) {
	...
	function getSingles() {
		...
		foreach ( $GLOBALS ['TYPO3_CONF_VARS'] ['EXTCONF'] ['synthesis'] ['sections'] as $sectionName => $sectionConf ) {
	...
	function view_result() {
		...
		if (! $this->EMmode) {
			...
			$content .= '
				...
				<select name="' . $this->piFieldName ( 'loc' ) . '">' . ($this->pObj->importAsType ( 'G' ) ? '<option value="G">Global: ' . $this->pObj->typePaths ['G'] . $this->saveKey . '/' . (@is_dir ( PATH_site . $this->pObj->typePaths ['G'] . $this->saveKey ) ? ' (OVERWRITE)' : ' (empty)') . '</option>' : '') . ($this->pObj->importAsType ( 'L' ) ? '<option value="L" selected="selected">Local: ' . $this->pObj->typePaths ['L'] . $this->saveKey . '/' . (@is_dir ( PATH_site . $this->pObj->typePaths ['L'] . $this->saveKey ) ? ' (OVERWRITE)' : ' (empty)') . '</option>' : '') . '</select>
				<input type="submit" name="' . $this->piFieldName ( 'WRITE' ) . '" value="WRITE" onclick="return confirm(\'If the setting in the selectorbox says OVERWRITE\nthen the marked files of the current extension in that location will be OVERRIDDEN! \nPlease decide if you want to continue.\n\n(Remember, this is a *synthesis* - NOT AN editor!)\');" />
			';
		}
...
// Include extension?
if (defined ( 'TYPO3_MODE' ) && $TYPO3_CONF_VARS [TYPO3_MODE] ['XCLASS'] ['ext/synthesis/class.tx_synthesis_wizard.php']) {
	include_once ($TYPO3_CONF_VARS [TYPO3_MODE] ['XCLASS'] ['ext/synthesis/class.tx_synthesis_wizard.php']);
}
