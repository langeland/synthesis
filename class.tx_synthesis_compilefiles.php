...
class tx_synthesis_compilefiles extends tx_synthesis_sectionbase {
	...
	function makeFilesArray($extKey) {
		...
		$this->addFileToFileArray ( 'Documentation/ChangeLog', date ( 'Y-m-d' ) . '  ' . $this->userField ( 'name' ) . '  <' . $this->userField ( 'email' ) . '>
...
' );
		...
		// icon:
		$this->addFileToFileArray ( 'ext_icon.gif', t3lib_div::getUrl ( t3lib_extMgm::extPath ( 'synthesis' ) . 'res/notfound.gif' ) );
	}

}

// Include extension?
if (defined ( 'TYPO3_MODE' ) && $TYPO3_CONF_VARS [TYPO3_MODE] ['XCLASS'] ['ext/synthesis/class.tx_synthesis_compilefiles.php']) {
	include_once ($TYPO3_CONF_VARS [TYPO3_MODE] ['XCLASS'] ['ext/synthesis/class.tx_synthesis_compilefiles.php']);
}
