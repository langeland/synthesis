 ...
 */
class tx_synthesis_section_cm extends tx_synthesis_sectionbase {
	var $sectionID = 'cm';

	/**
	 * Renders the form in the synthesis; this was add_cat_cm()
	 *
	 * @return	HTML code
	 */
	...
	function render_extPart($k,$config,$extKey) {
		...
		$this->addLocalConf($this->wizard->ext_locallang,$config,'title','cm',$k);
			...
			// Add icon
		$this->addFileToFileArray($pathSuffix.'cm_icon.gif',t3lib_div::getUrl(t3lib_extMgm::extPath('synthesis').'res/notfound_module.gif'));
		...
		if ($config['second_level'])	{
			...
			$this->addLocalConf($this->wizard->ext_locallang,array('title_activate'=>'...Second level ->'),'title_activate','cm',$k,0,1);
				// Add activate icon
			$this->addFileToFileArray($pathSuffix.'cm_icon_activate.gif',t3lib_div::getUrl(t3lib_extMgm::extPath('synthesis').'res/notfound_module.gif'));
		}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/synthesis/sections/class.tx_synthesis_section_cm.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/synthesis/sections/class.tx_synthesis_section_cm.php']);
}
