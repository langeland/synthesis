...
require_once (t3lib_extMgm::extPath ( 'synthesis' ) . 'sections/class.tx_synthesis_section_fields.php');
class tx_synthesis_section_model extends tx_synthesis_section_fields {
	var $sectionID = 'model';


	/**
	 * Renders the form in the synthesis; this was add_cat_tables()
	 *
	...
	function render_wizard() {
		...
		if ($action [0] == 'edit') {
			...
			$subContent = '<strong>Tablename:</strong><BR>' . $this->returnName ( $this->wizard->extKey, 'tables' ) . '_' . $this->renderStringBox ( $ffPrefix . '[tablename]', trim ( $piConf ['tablename'] ) ) . '<BR><strong>Notice:</strong> Use characters a-z0-9 only. Only lowercase, no spaces.<BR>
				This becomes the table name in the database. ';
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			

			// Enter title of the table
			$subContent = '<strong>Title of the table:</strong><BR>' . $this->renderStringBox_lang ( 'title', $ffPrefix, $piConf );
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			...
			/*******************************************************************************/
			// $lines[] = '<tr' . $this->bgCol ( 3 ) . '><td><h3 class="divider">Domain object settings</h3></td></tr>';
			
			$subContent = '<strong>Domain object settings:</strong><br />Use icon from the following extension';
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			
			$subContent = $this->renderCheckBox ( $ffPrefix . '[createDomain]', $piConf ['createDomain'] );
			$subContent .= 'Create domain';
			$subContent .= $this->whatIsThis ( 'This will create a model and a reposetory.' );
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			
			$subContent = $this->renderCheckBox ( $ffPrefix . '[isAggregateRoot]', $piConf ['isAggregateRoot'] );
			$subContent .= 'Is aggregate root';
			$subContent .= $this->whatIsThis ( 'If true, the extension builder will generate a repository and default controller actions for this model.' );
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			...
			$subContent = 'Object type:<br />';
			$subContent .= $this->renderSelectBox ( $ffPrefix . '[objectType]', $piConf ['objectType'], $optValues );
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			...
			$subContent .= $this->textSetup ( '', 'If "Manual ordering" is not set, order the table by this field:<BR>' . $this->renderSelectBox ( $ffPrefix . '[sorting_field]', $piConf ['sorting_field'], $this->currentFields ( $optValues, $piConf ['fields'] ) ) . '<BR>' . $this->renderCheckBox ( $ffPrefix . '[sorting_desc]', $piConf ['sorting_desc'] ) . ' Descending' );
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			...
			$subContent = '<strong>"Type-field", if any:<BR></strong>' . $this->renderSelectBox ( $ffPrefix . '[type_field]', $piConf ['type_field'], $this->currentFields ( $optValues, $piConf ['fields'] ) ) . $this->whatIsThis ( 'A "type-field" is the field in the table which determines how the form is rendered in the backend, eg. which fields are shown under which circumstances. For instance the Content Element table "tt_content" has a type-field, CType. The value of this field determines if the editing form shows the bodytext field as is the case when the type is "Text" or if also the image-field should be shown as when the type is "Text w/Image"' );
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			...
			$subContent = '<strong>Label-field:<BR></strong>' . $this->renderSelectBox ( $ffPrefix . '[header_field]', $piConf ['header_field'], $this->currentFields ( $optValues, $piConf ['fields'] ) ) . $this->whatIsThis ( 'A "label-field" is the field used as record title in the backend.' );
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			...
			// Allowed on pages
			$subContent = '<strong>Allowed on pages:<BR></strong>' . $this->renderCheckBox ( $ffPrefix . '[allow_on_pages]', $piConf ['allow_on_pages'] ) . ' Allow records from this table to be created on regular pages.';
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			
			// Allowed in "Insert Records"
			$subContent = '<strong>Allowed in "Insert Records" field in content elements:<BR></strong>' . $this->renderCheckBox ( $ffPrefix . '[allow_ce_insert_records]', $piConf ['allow_ce_insert_records'] ) . ' Allow records from this table to be linked to by content elements.';
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			
			// Add new button
			$subContent = '<strong>Add "Save and new" button in forms:<BR></strong>' . $this->renderCheckBox ( $ffPrefix . '[save_and_new]', $piConf ['save_and_new'] ) . ' Will add an additional save-button to forms by which you can save the item and instantly create the next.';
			$lines [] = '<tr' . $this->bgCol ( 3 ) . '><td>' . $this->fw ( $subContent ) . '</td></tr>';
			

			$subContent = '<strong>Notice on fieldnames:<BR></strong>' . 'Don\'t use fieldnames from this list of reserved names/words: <BR>
		...
		if (is_array ( $GLOBALS ['TYPO3_CONF_VARS'] ['EXTCONF'] ['synthesis'] ['add_cat_tables'] )) {
			foreach ( $GLOBALS ['TYPO3_CONF_VARS'] ['EXTCONF'] ['synthesis'] ['add_cat_tables'] as $_funcRef ) {
	...
	function renderFieldOverview($prefix, $fConf, $dontRemove = 0) {
		...
		return sprintf ( '<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td>', $fConf ['fieldname'], $fConf ['title'], $optTypes [$fConf ['type']], $fConf ['exludeField'] ? 'Yes' : '', $details );
	}
	...
	function render_extPart($k, $config, $extKey) {
		...
		$modelName = t3lib_div::underscoredToUpperCamelCase ( trim ( $config ['tablename'] ) );
		...
		if ($config ['createDomain']) {
			$this->renderDomain ( $k, $config, $extKey );
		}
		...
		$ctrl [] = '\'dynamicConfigFile\' => t3lib_extMgm::extPath($_EXTKEY) . \'Configuration/TCA/' . $modelName . '.php\',';
		...
		$this->addFileToFileArray ( 'Configuration/TCA/' . $modelName . '.php', $tca_file );
	...
	function renderDomain($k, $config, $extKey) {
		$modelName = ucfirst ( $config ['tablename'] );
		$modelName = t3lib_div::underscoredToUpperCamelCase ( trim ( $config ['tablename'] ) );
		 ...
		 */
		$modelPart = array ();
		$modelClassName = $this->returnClassName ( $extKey, 'model', $modelName );
		$modelPart [] = 'class ' . $modelClassName . ' extends Tx_Extbase_DomainObject_AbstractEntity {';
		

		$modelPartDefine = array ();
		$modelPartFunction = array ();
		if (is_array ( $config ['fields'] )) {
			reset ( $config ['fields'] );
			foreach ( $config ["fields"] as $field ) {
				$modelPartDefine [] = $this->makeFieldDefine ( $field );
				$modelPartFunction [] = $this->makeFieldFunktion ( $field );
			}
		}
		...
		$modelPart [] = '}'; // Close class
		
		$modelFileName = 'Classes/Domain/Model/' . $modelName . '.php';
		$modelContent = $this->PHPclassFile ( $extKey, $modelFileName, implode ( "\n", $modelPart ), 'Description' );
		$this->addFileToFileArray ( $modelFileName, $modelContent );
		
		/*
		 * Render Repository
		 */
		$repositoryPart = array ();
		$repositoryClassName = $this->returnClassName ( $extKey, 'repository', $modelName );
		$repositoryPart [] = 'class ' . $repositoryClassName . ' extends Tx_Extbase_Persistence_Repository {';
		
		$repositoryPart [] = '}';
		
		$repositoryFileName = 'Classes/Domain/Repository/' . $modelName . 'Repository.php';
		$repositoryContent = $this->PHPclassFile ( $extKey, $repositoryFileName, implode ( "\n", $repositoryPart ), 'Description' );
		$this->addFileToFileArray ( $repositoryFileName, $repositoryContent );
	}


	function makeFieldDefine($field) {
		$fieldDefine = array ();
		
		$fieldDefine [] = '/*';
		$fieldDefine [] = print_r ( $field, 1 );
		$fieldDefine [] = '*/';
		$fieldDefine [] = "\n\n";
		

		$fieldDefine [] = '/**';
		$fieldDefine [] = ' * ' . $field ['title'];
		$fieldDefine [] = ' *';
		$fieldDefine [] = ' * @var '.$this->getVarType($field);
		if ($field['conf_required']) {
			$fieldDefine [] = ' * @validate NotEmpty';
		}
		$fieldDefine [] = ' */';
		$fieldDefine [] = 'protected $' . $field ['fieldname'] . ';';
		$fieldDefine [] = "\n";
	...
	function makeFieldFunktion($field) {
		
		$fieldFunktion = array ();
		
		$lowerName = t3lib_div::underscoredToLowerCamelCase($field['fieldname']);
		$upperName = t3lib_div::underscoredToUpperCamelCase($field['fieldname']);
		
		
		
		$fieldFunktion [] = '/**';
		$fieldFunktion [] = ' * Returns the '.$field ['title'];
		$fieldFunktion [] = ' *';
		$fieldFunktion [] = ' * @return '.$this->getVarType($field).' $'.$lowerName;
		$fieldFunktion [] = ' */';
		$fieldFunktion [] = 'public function get'.$upperName.'() {';
		$fieldFunktion [] = '	return $this->'.$lowerName.';';
		$fieldFunktion [] = '}';
		$fieldFunktion [] = '';
		$fieldFunktion [] = '/**';
		$fieldFunktion [] = ' * Sets the '.$field ['title'];
		$fieldFunktion [] = ' *';
		$fieldFunktion [] = ' * @param '.$this->getVarType($field).' $'.$lowerName;
		$fieldFunktion [] = ' * @return void';
		$fieldFunktion [] = ' */';
		$fieldFunktion [] = 'public function set'.$upperName.'($'.$lowerName.') {';
		$fieldFunktion [] = '	$this->'.$lowerName.' = $'.$lowerName.';';
		$fieldFunktion [] = '}';
...
if (defined ( 'TYPO3_MODE' ) && $TYPO3_CONF_VARS [TYPO3_MODE] ['XCLASS'] ['ext/synthesis/sections/class.tx_synthesis_section_tsconfig.php']) {
	include_once ($TYPO3_CONF_VARS [TYPO3_MODE] ['XCLASS'] ['ext/synthesis/sections/class.tx_synthesis_section_tsconfig.php']);
}
?>