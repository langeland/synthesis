 ...
 */
class tx_synthesis_section_fields extends tx_synthesis_sectionbase {
	var $sectionID = 'fields';


	/**
	 * Renders the form in the synthesis; this was add_cat_fields()
	 *
	...
	function render_wizard() {
		...
		if (is_array ( $GLOBALS ['TYPO3_CONF_VARS'] ['EXTCONF'] ['synthesis'] ['add_cat_fields'] )) {
			foreach ( $GLOBALS ['TYPO3_CONF_VARS'] ['EXTCONF'] ['synthesis'] ['add_cat_fields'] as $_funcRef ) {
	...
	function presetBox(&$piConfFields) {
		...
		// Session presets:
		$ses_optValues = array ();
		$sesdat = $GLOBALS ['BE_USER']->getSessionData ( 'synthesis' );
		...
		if (count ( $ses_optValues )) {
			$optValues = array_merge ( $optValues, count ( $optValues ) ? array (
					'<option value=""></option>' 
			) : array (), array (
					'<option value="">__Fields picked up in this session__:</option>' 
			), $ses_optValues );
		}
		if (count ( $optValues )) {
	...
	function cleanFieldsAndDoCommands($fConf, $catID, $action, $table) {
		...
		$GLOBALS ['BE_USER']->setAndSaveSessionData ( 'synthesis', $sesdat );
	...
	function renderField($prefix, $fConf, $dontRemove = 0) {
		...
		$fieldName = $this->renderStringBox ( $prefix . '[fieldname]', $fConf ['fieldname'] ) . (! $dontRemove ? ' (Remove:' . $this->renderCheckBox ( $prefix . '[_DELETE]', 0 ) . ')' . '<input type="image" hspace="2" src="' . $this->wizard->siteBackPath . TYPO3_mainDir . 'gfx/pil2up.gif" name="' . $this->varPrefix . '_CMD_' . $fConf ["fieldname"] . '_UP" onclick="' . $onCP [1] . '" />' . '<input type="image" hspace="2" src="' . $this->wizard->siteBackPath . TYPO3_mainDir . 'gfx/pil2down.gif" name="' . $this->varPrefix . '_CMD_' . $fConf ["fieldname"] . '_DOWN" onclick="' . $onCP [1] . '" />' . '<input type="image" hspace="2" src="' . $this->wizard->siteBackPath . TYPO3_mainDir . 'gfx/savesnapshot.gif" name="' . $this->varPrefix . '_CMD_' . $fConf ["fieldname"] . '_SAVE" onclick="' . $onCP [1] . '" title="Save this field setting as a preset." />' : '');
		...
		switch (( string ) $fConf ['type']) {
			...
			case 'textarea_rte' :
				...
				if (( string ) $fConf ['conf_rte'] == 'custom') {
					...
					for($a = 1; $a < 7; $a ++) {
						$fDetails .= 'Class Title:' . $this->renderStringBox ( $prefix . '[conf_rte_class' . $a . ']', $fConf ['conf_rte_class' . $a], 100 ) . '<br />CSS Style: {' . $this->renderStringBox ( $prefix . '[conf_rte_class' . $a . '_style]', $fConf ['conf_rte_class' . $a . '_style'], 250 ) . '}' . '<br />';
					}
			...
			case 'rel' :
				...
				if ($fConf ['conf_rel_table'] == '_CUSTOM')
					$fDetails .= 'Custom table name: ' . $this->renderStringBox ( $prefix . '[conf_custom_table_name]', $fConf ['conf_custom_table_name'], 200 ) . '<br />';
		...
		switch ($t) {
			...
			case 'textarea_nowrap' :
				...
				if ($fConf ["conf_wiz_example"]) {
					...
					$this->writeStandardBE_xMod ( $extKey, array (
							'title' => 'Example Wizard title...' 
					), $id . '/', $cN, 0, $id . 'wiz' );
					$this->addFileToFileArray ( $id . '/wizard_icon.gif', t3lib_div::getUrl ( t3lib_extMgm::extPath ( 'synthesis' ) . 'res/notfound.gif' ) );
			...
			case 'select' :
				...
				for($a = 0; $a < $numberOfItems; $a ++) {
					...
					if ($fConf ["conf_select_icons"] && $t == "select") {
						...
						// Add wizard icon
						$this->addFileToFileArray ( "selicon_" . $id . "_" . $a . ".gif", t3lib_div::getUrl ( t3lib_extMgm::extPath ( "synthesis" ) . "res/wiz.gif" ) );
					} else
						$icon = "";
...
*/
	...
	function renderInlineField($prefix, $fConf) {
		...
		if ($fConf ['foreign_table'] != 'blank' && $fConf ['foreign_table'] != '_CUSTOM' || $fConf ['conf_custom_table_name'] != '') {
			...
			$fDetails .= '<br /><strong>Relation Type:</strong><br />' . $this->renderSelectBox ( $prefix . '[relation_type]', $fConf ['relation_type'], array (
					'1-n' => '1:N',
					'm-n' => 'M:N' 
			) ) . '<br />';
			
			$fDetails .= '<br /><strong>Foreign field:</strong><br />' . $this->renderSelectBox ( $prefix . '[foreign_field]', $fConf ['foreign_field'], $this->getFieldsFromTable ( $fConf ['foreign_table'] ) );
		...
		if ($fConf ['foreign_field']) {
			foreach ( $this->wizard->wizArray ['tables'] as $k => $v ) {
				...
				if (preg_match ( '/' . $this->wizard->wizArray ['tables'] [$k] ['tablename'] . '/', $fConf ['foreign_table'] )) {
					...
					if (! count ( $match )) {
						$this->wizard->wizArray ['tables'] [$k] ['fields'] [$count + 1] = array (
								'fieldname' => $fConf ['foreign_field'],
								'type' => 'passthrough' 
						);
					}
				}
		...
		if ($fConf ['foreign_table_field']) {
			foreach ( $this->wizard->wizArray ['tables'] as $k => $v ) {
				...
				if (preg_match ( '/' . $this->wizard->wizArray ['tables'] [$k] ['tablename'] . '/', $fConf ['foreign_table'] )) {
					...
					if (! count ( $match )) {
						$this->wizard->wizArray ['tables'] [$k] ['fields'] [$count + 1] = array (
								'fieldname' => $fConf ['foreign_table_field'],
								'type' => 'passthrough' 
						);
					}
				}
		...
		if ($fConf ['foreign_sortby']) {
			foreach ( $this->wizard->wizArray ['tables'] as $k => $v ) {
				...
				if (preg_match ( '/' . $this->wizard->wizArray ['tables'] [$k] ['tablename'] . '/', $fConf ['foreign_table'] )) {
					...
					if (! count ( $match )) {
						$this->wizard->wizArray ['tables'] [$k] ['fields'] [$count + 1] = array (
								'fieldname' => $fConf ['foreign_sortby'],
								'type' => 'passthrough' 
						);
					}
				}
		...
		if ($fConf ['foreign_selector']) {
			foreach ( $this->wizard->wizArray ['tables'] as $k => $v ) {
				...
				if (preg_match ( '/' . $this->wizard->wizArray ['tables'] [$k] ['tablename'] . '/', $fConf ['foreign_selector'] )) {
					...
					if (! count ( $match )) {
						$this->wizard->wizArray ['tables'] [$k] ['fields'] [$count + 1] = array (
								'fieldname' => $fConf ['foreign_selector'],
								'type' => 'select' 
						);
					}
				}
		...
		if ($fConf ['foreign_unique']) {
			foreach ( $this->wizard->wizArray ['tables'] as $k => $v ) {
				...
				if (preg_match ( '/' . $this->wizard->wizArray ['tables'] [$k] ['tablename'] . '/', $fConf ['foreign_unique'] )) {
					...
					if (! count ( $match )) {
						$this->wizard->wizArray ['tables'] [$k] ['fields'] [$count + 1] = array (
								'fieldname' => $fConf ['foreign_unique'],
								'type' => 'select' 
						);
					}
				}
			...
			if ($fConf ['conf_symmetric_field']) {
				foreach ( $this->wizard->wizArray ['tables'] as $k => $v ) {
					...
					if (preg_match ( '/' . $this->wizard->wizArray ['tables'] [$k] ['tablename'] . '/', $fConf ['conf_symmetric_field'] )) {
						...
						if (! count ( $match )) {
							$this->wizard->wizArray ['tables'] [$k] ['fields'] [$count + 1] = array (
									'fieldname' => $fConf ['conf_symmetric_field'],
									'type' => 'select' 
							);
						}
					}
			...
			if ($fConf ['conf_symmetric_sortby']) {
				foreach ( $this->wizard->wizArray ['tables'] as $k => $v ) {
					...
					if (preg_match ( '/' . $this->wizard->wizArray ['tables'] [$k] ['tablename'] . '/', $fConf ['conf_symmetric_sortby'] )) {
						...
						if (! count ( $match )) {
							$this->wizard->wizArray ['tables'] [$k] ['fields'] [$count + 1] = array (
									'fieldname' => $fConf ['conf_symmetric_sortby'],
									'type' => 'passthrough' 
							);
						}
					}
...
if (defined ( 'TYPO3_MODE' ) && $TYPO3_CONF_VARS [TYPO3_MODE] ['XCLASS'] ['ext/synthesis/sections/class.tx_synthesis_section_fields.php']) {
	include_once ($TYPO3_CONF_VARS [TYPO3_MODE] ['XCLASS'] ['ext/synthesis/sections/class.tx_synthesis_section_fields.php']);
}
